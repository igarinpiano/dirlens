...
fn tool_defs() -> Value {
    ...
    json!([
        ...
        {
            "name": "focus",
            "description": "Impact analysis for one file: what it imports and what imports it (direct + transitive), as JSON. Use before refactoring to see the blast radius, and again afterward — or before any later work that touches the same area — to keep that picture current; it's cheap enough to call repeatedly through a session rather than once per task. Nested Rust crates (Cargo.toml in a subdirectory) are resolved automatically, so calling from the repo root is fine for Rust. For JS/TS/Go files inside a nested sub-project (its own tsconfig.json/package.json/go.mod), alias resolution is rooted at `path` — the result carries a `note` field when this might make it incomplete; in that case re-run with `path` set to that subdirectory.",
            "inputSchema": obj(json!({"path": path_prop, "file": {"type": "string", "description": "project-relative source file"}}), vec!["file"])
        },
        {
            "name": "todos",
            "description": "TODO/FIXME/HACK/XXX comments across the project, as a flat JSON list of {path, line, kind, text} — only files that actually have one are included (files with none are omitted, unlike `analyze`/`tree` annotations — cheap on any project size, no `depth`/`budget` needed). Cheap enough to re-check periodically over a long session, not just once, since edits (yours or others') keep adding and resolving TODOs.",
            "inputSchema": obj(json!({
                "path": path_prop,
        ...
        {
            "name": "since",
            "description": "Only the files changed since a git ref (default HEAD), as a JSON tree annotated with tokens, outline, and TODOs per changed file. Much cheaper than re-running analyze, so use this liberally throughout a session — after every meaningful edit or batch of edits, not only when something feels stale — to keep your picture of the project current as you work.",
            "inputSchema": obj(json!({
                "path": path_prop,
