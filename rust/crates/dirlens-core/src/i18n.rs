...
pub struct Texts {
    ...
    /// "exact BPE (o200k_base); files over {size} are extrapolated" の {size} 直前まで。
    pub check_tokens_bpe_prefix: &'static str,
    /// {size} 直後の "are extrapolated" 相当部分。
    pub check_tokens_bpe_suffix: &'static str,
    ...
    /// git は使えるが対象ディレクトリが git repo ではない場合の案内
    /// （dirlens/環境の能力不足ではなく対象ディレクトリの事実なので degraded には含めない）。
    pub gitignore_not_a_repo_hint: &'static str,
...
pub static EN: Texts = Texts {
    ...
    check_tokens_bpe_prefix: "exact BPE (o200k_base); files over ",
    check_tokens_bpe_suffix: " are extrapolated",
    ...
    gitignore_not_a_repo_hint: "not a git repository yet (git itself is available; git init would enable exact matching)",
...
pub static JA: Texts = Texts {
    ...
    check_tokens_bpe_prefix: "BPE（o200k_base）による正確値（",
    check_tokens_bpe_suffix: " 超は比例概算）",
    check_tokens_heuristic: "文字数ベースの概算",
    check_all_best: "\nすべての機能が最良の方式で動作します。\n",
    check_degraded_header: "\n縮退している項目:\n",
    deg_no_git: "git が見つからない（-H 不可・gitignore は内蔵マッチャ）",
    gitignore_not_a_repo_hint: "対象は git リポジトリではない（git 自体は利用可能。git init で厳密一致が有効に）",
