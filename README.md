...
- **MCP サーバー** — `--mcp` で MCP サーバーとして起動し、analyze / tree / outline / imports / focus / todos の6ツールをエージェントがネイティブに呼べる。**設定は `dirlens --mcp-setup` が案内**（バイナリの絶対パス入りで Claude Code のワンライナー・Claude Desktop / Cursor の設定 JSON を出力するので、コピペで完了）
