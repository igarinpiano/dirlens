...
- **トークン予算** — `--budget N` でテキスト出力自体を指定トークン数以内に自動調整（自前の BPE で実測しながら深さ・詳細を削減）
...
- **MCP サーバー** — `--mcp` で MCP サーバーとして起動。`claude mcp add dirlens -- dirlens --mcp` で登録すると analyze / tree / outline / imports / focus / todos の6ツールをエージェントがネイティブに呼べる
...
| `--budget N`        | —            | テキスト出力を約 N トークン以内に自動調整（深さ→詳細の順で削減）  |
