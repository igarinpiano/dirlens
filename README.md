...
- ディレクトリサイズの計算（合計のための全サブツリー stat）は **ツリー全体をマルチコアで並列走査** して高速化（native ビルドのみ・透過的な最適化）。レベル同期の並列 BFS で全ディレクトリの直下サイズを分散収集し、ボトムアップで合計を畳み込む。巨大な単一サブツリー（例: `~/Library/Caches`）があっても複数スレッドで分担するため負荷が偏らない。syscall（I/O）律速なので効果はストレージ依存だが、実ディスクでのフラグ無し実行が体感的に速くなる。出力は直列計算と完全にバイト一致し、単一コアでは退行しない（`DIRLENS_MAX_WORKERS` でスレッド数を調整可）
- **重い解析（トークン計数・アウトライン・TODO・import 解析）はマルチコアで並列実行**して高速化（native ビルドのみ・透過的な最適化）。本文の読み込み・BPE トークナイズ・AST パースはファイル内容だけで結果が決まる純粋な計算のため、走査前に全ファイル分を並列で先に計算してからレンダリングする。出力は直列実行と完全にバイト一致する（`--agent` / `-T` / `-O` / `-M` などが 4 コアでおよそ 3 倍高速）
...
- **[`AGENT_RULE_MCP.md`](AGENT_RULE_MCP.md)** — CLIとMCPサーバー（`--mcp`）を両方登録済みの環境向け。同じ情報を取る手段が2つあるときの使い分け（どちらを優先するか、片方が不調な時のフォールバック等）を追加した版
...
├── crates/dirlens-cli/    # CLI（clap・std プロバイダ・並列サイズウォーム）
