...
| git 連携（`-H`/`--status`/`--since`/`--api-diff`） | git コマンド実行（`-H` は直近 2000 コミットを走査）。リポジトリのサブディレクトリをスキャンルートに指定した場合も、git のパス（リポジトリルート相対）をスキャンルート相対へ変換して正しく突き合わせる（v1.2.8+） | — | それより古い変更しかないファイルは情報が出ない。v1.2.7 以前はサブディレクトリ指定時にコミット注釈の欠落・同名ファイルの誤った履歴・`--since` の空振り・`--api-diff` の全削除誤報が起きた |
