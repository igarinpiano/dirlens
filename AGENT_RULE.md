...
5. **`-L`（深さ制限）はツリー表示を浅くするだけで、集計（TODO件数・テスト未整備数・エントリーポイント数・依存度ランキング等）はプロジェクト全体のスキャン結果を反映する**
   深い階層の概要だけサッと見たいときは `-L 2` 等を使ってよいが、ツリーの「見た目」が浅くなるだけで、サマリーの数値自体は省略されない（修正済み）。
...
| `env: node: No such file or directory` | （旧バージョン限定）npm wrapperの起動にNodeが必要だった | dirlensを最新版に更新する。最新版は `dirlens` コマンド自体が `dirlens.py` への直接リンクになっており、実行時にNodeを必要としない |
| 実行しても無言で exit 1（何も出力されない） | （旧バージョン限定）`python3` がPATHに無く、wrapperがエラーを表示せず終了していた | 最新版に更新する（修正済み）。更新できない場合は `python3 <dirlens.pyへのパス> --agent` を直接試す |
| `Fatal Python error: Failed to import encodings module` | `python3` が指している実体（pyenv/mise/conda管理のPython等）がサンドボックスから読み取れない | システムのPython3を明示的に指定する: `/usr/bin/python3 <パス> --agent`（macOS）。**ただしこれでも `Operation not permitted` になる場合は次の行へ** |
