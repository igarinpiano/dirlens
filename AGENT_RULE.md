...
このテンプレートは、dirlensが**入っていない/古いかもしれない**環境向け。存在確認とフォールバック手順を重視する。dirlensのCLIが常時使える前提でよいなら [AGENT_RULE_STRICT.md](AGENT_RULE_STRICT.md)、CLIとMCPサーバー（`--mcp`）を両方登録済みの環境なら [AGENT_RULE_MCP.md](AGENT_RULE_MCP.md) を使ってください。
...
dirlensは、コードベースを探索する際に `ls`/`find`/`tree`/`grep` を何度も打つ代わりに使う単一バイナリのCLIツール。ファイルツリーに加えて、正確なトークン数（BPE）、gitの最終コミット情報、TODO/FIXME一覧、テスト未整備ファイル、エントリーポイント候補、関数/クラスのアウトライン、import依存グラフ、設定ファイル一覧などを一度の実行でまとめて出力できる。ただし**このテンプレートでは「使えたら速い索引ツール」として扱う。必須ツールとして扱わない**。実行に失敗しても作業を止めず、標準コマンドにフォールバックして調査を続けること。
...
| 機能 | 方式と制限事項 |
|---|---|
| トークン数（`-T`） | BPE（o200k_base）による正確値。1ファイルあたりの読み込み上限（既定5MB）を超えると比例概算（JSON では `tokens_estimated: true` が付く・v1.2.9+）。この上限は v1.2.17+ でホストの物理メモリ量に応じて段階的に引き上がる（8GB未満は5MBのまま、16/32/64GB刻みで15/30/50/80MB。`DIRLENS_MAX_FILE_BYTES` で明示指定も可）。実際の値は `--check`/`--agent --json` の `capabilities.max_file_bytes` で確認できる。他社モデルのトークナイザでは目安。通常ファイル以外（FIFO・ソケット・デバイス）は読まずサイズ 0 扱い（v1.2.12+。v1.2.11 以前は FIFO を含むディレクトリで永久にブロックした） |
